...
3. Run the installed plugin's catalog command. Prefer repo-local `docs:governor:*` scripts when available and runnable in the current shell.

If repo-local scripts are not available, or if they exist but are PowerShell-only and the current macOS/Linux environment has no `pwsh` or `powershell`, use the core Node script directly from the installed plugin directory. When this skill is loaded from an installed plugin, resolve `/path/to/documentation-governor` from the parent plugin directory that contains this `SKILL.md`; the Node script lives at `scripts/docs-governor.mjs`.
...
4. Inspect the current governance status. Prefer the repo-local `docs:check` command when available and runnable so project-specific stale scans run before the governor. If it is PowerShell-only and PowerShell is unavailable, run the Node check directly instead of skipping the gate. Otherwise use the Node command or PowerShell wrapper:
...
When checking uncommitted work, include the working-tree changed files explicitly because the default diff range may only inspect committed changes:

```bash
(git diff --name-only origin/main...HEAD; git diff --name-only; git diff --name-only --cached) | sort -u > /tmp/docs-governor-changed-files.txt
node /path/to/documentation-governor/scripts/docs-governor.mjs check --config ./.documentation-governor.json --files-file /tmp/docs-governor-changed-files.txt
```

5. Read the changed code and the existing docs.
6. Update the human-readable docs under `docs/`.
7. Record the refresh. Prefer repo-local `docs:governor:refresh` when available and runnable. Otherwise use the Node command or PowerShell wrapper:
